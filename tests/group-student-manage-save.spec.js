...
test('관리자가 그룹 학생 관리 모달에서 제외 날짜를 저장하고 다시 원복할 수 있다', async ({
  ...
  const studentRow = groupStudentsSection
    .locator('.table-row')
  ...
  async function ensureDateRemovedIfPresent() {
    ...
    const dateItem = dialog.locator('li').filter({ hasText: excludedDate }).first();
  ...
  try {
    ...
    const excludeDateInput = addDialog.locator('input[type="date"]').nth(1);
    ...
    await expect(addDialog.locator('li').filter({ hasText: excludedDate }).first()).toBeVisible();
    ...
    await expect(
      verifyAddedDialog.locator('li').filter({ hasText: excludedDate }).first()
    ).toBeVisible();

    await verifyAddedDialog
      .locator('li')
      .filter({ hasText: excludedDate })
      .first()
    ...
    await expect(
      verifyRemovedDialog.locator('li').filter({ hasText: excludedDate }).first()
    ).toHaveCount(0);
    await expect(
    ...
    await expect(
      verifyCleanupDialog.locator('li').filter({ hasText: excludedDate }).first()
    ).toHaveCount(0);
